...
def make_upload(imputer, model=LGBMRegressor(random_state=42), scaler=None, status=False, test_size=0.15, seed=42):
    ...
    for i in range(1,8,1):
        if imputer is not None:
            np_train, np_answer = np.array(eval(f'r.train{i}')), np.array(eval(f'r.answer{i}'))
            ...
            np_train, np_answer = np.array(eval(f'r.temp_train{i}')), np.array(eval(f'r.answer{i}'))
        ...
        if status is True:
            if imputer is not None:
                X_test = np.array(eval(f'r.test{i}'))
            ...
            if imputer is not None:
                ...
                upload.to_csv(f'upload/Forest_full_{str(model)[:4]}_upload{i}.csv', header=None, index=None, sep=',', mode='w')
        print()
